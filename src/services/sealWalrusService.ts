...
export class SealWalrusService {
  ...
  async loadProfile(
    ...
    try {
      ...
      try {
        const [profileObj, recordsObj] = await Promise.all([
          this.suiClient.getObject({ id: profileObjectId }),
          this.suiClient.getObject({ id: recordsObjectId }),
        ]);

        if (!profileObj.data || !recordsObj.data) {
          console.error(
            "Profile or Records object not found or not accessible"
          );
          return null;
        }
